...
from finbot_common.settings import entry_allowed_key, entry_regimes_key, get_bool, get_float, get_int, parse_regimes_list
...
class LemeEngine:
    ...
    def _allowed_regimes(self, settings: dict, direction: str, tier: str) -> list[str]:
        key = entry_regimes_key(direction, tier)
        fallback_key = f"entry.{direction.lower()}_allowed_regimes"
        default = ["bull"] if direction.upper() == "LONG" else ["bear", "neutral"]
        regimes = settings.get(key)
        if regimes is None:
            regimes = settings.get(fallback_key, default)
        return parse_regimes_list({key: regimes}, key, default)
        ...
        try:
            ...
            for asset in assets:
                ...
                for direction, strategy, score, _has_model in candidates:
                    ...
                    allowed = self._allowed_regimes(settings, direction, tier)
            ...
            if orders_published:
                logger.info("Publicadas %s ordens em trade.order.v1", orders_published)
