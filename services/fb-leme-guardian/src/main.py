...
from finbot_common.settings import (
    entry_allowed_key,
    entry_regimes_backup_key,
    entry_regimes_key,
    get_bool,
    get_float,
    get_int,
...
DEFAULT_TIER_REGIMES = {
    ("long", "Major"): ["bull"],
    ("long", "Strong Alt"): ["bull"],
    ("long", "High Volatility"): ["bull"],
    ("short", "Major"): ["bear", "neutral"],
    ("short", "Strong Alt"): ["bear", "neutral"],
    ("short", "High Volatility"): ["bear", "neutral"],
}
...
class LemeGuardian:
    ...
    def apply_disable(self, conn, direction: str, tier: str, reason: str, settings: dict):
        allowed_key = entry_allowed_key(direction, tier)
        regimes_key = entry_regimes_key(direction, tier)
        backup_key = entry_regimes_backup_key(direction, tier)

        current_regimes = parse_regimes_list(
            settings,
            regimes_key,
            DEFAULT_TIER_REGIMES.get((direction, tier), []),
        )
        upsert_block_setting(conn, BLOCK_ID, backup_key, current_regimes)
        upsert_block_setting(conn, BLOCK_ID, regimes_key, [])
        upsert_block_setting(conn, BLOCK_ID, allowed_key, False)
        settings[backup_key] = current_regimes
        settings[regimes_key] = []
    ...
    def apply_enable(self, conn, direction: str, tier: str, reason: str, settings: dict):
        allowed_key = entry_allowed_key(direction, tier)
        regimes_key = entry_regimes_key(direction, tier)
        backup_key = entry_regimes_backup_key(direction, tier)

        restored = settings.get(backup_key)
        if restored is None:
            restored = DEFAULT_TIER_REGIMES.get((direction, tier), [])
        upsert_block_setting(conn, BLOCK_ID, regimes_key, restored)
        upsert_block_setting(conn, BLOCK_ID, allowed_key, True)
        settings[regimes_key] = restored
